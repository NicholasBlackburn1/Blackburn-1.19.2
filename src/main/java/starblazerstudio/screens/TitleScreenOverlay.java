...
public class TitleScreenOverlay {
   ...
   public void setCopyRightTexT(String splash, Minecraft minecraft, int copyrightWidth, int copyrightX, int width,
   ...
   public void drawCopyRightString(Screen screen, PoseStack pose, Font font, String copyrightinfo, int height,
         int width, int copyrightX, int copyrightWidth, int p_96740_, int p_96741_, float f1) {
      int l = Mth.ceil(f1 * 255.0F) << 24;

      screen.drawString(pose, font, copyrightinfo, copyrightX, height - 10, 16777215 | l);

      if (p_96740_ > copyrightX && p_96740_ < copyrightX + copyrightWidth && p_96741_ > height - 10
            && p_96741_ < height) {
   ...
   public void setSplashPos(Screen screen, String Splash, PoseStack p_96739_, int width, int splashX, int splashRot,
         Font font, int l) {
      if (Splash != null) {
         p_96739_.pushPose();
         ...
         float f2 = 1.8F
               - Mth.abs(Mth.sin((float) (Util.getMillis() % 1000L) / 1000.0F * ((float) Math.PI * 2F)) * 0.1F);
    ...
    *                         TODO: need to add Boolean for full screen or not to
    *                         set edition to the right hight
    */
   public void renderEdition(Screen screen, String splash, PoseStack p_96739_, Font font, int width, int hight,
         int editionXfull, int editionYfull, int splashX, int j, int l, int editionXSmol, int editionYSmol,
         int editionImageWidth, int editionImageHight, int editionTextureWidth, int splashrot, boolean isfullscreen) {
      int x = 0;
      int y = 0;
   ...
   // This allows me to load and displays all the buttons and the logo ans the
   // splashcreen all from the file
   public void LoadCustomMainMenu(Minecraft minecraft, Screen titlescreen, int buttonwidth, int j) {
      ...
      gui.CreatebuttonwithoutImage(minecraft, titlescreen, new SelectWorldScreen(titlescreen), buttonwidth, j,
            x.getAsInt(), y.getAsInt(), 20, "menu.singleplayer");

      x = (JsonPrimitive) Consts.background.get(6);
      y = (JsonPrimitive) Consts.background.get(5);
      gui.CreatebuttonwithoutImage(minecraft, titlescreen, new JoinMultiplayerScreen(titlescreen), buttonwidth, j,
            x.getAsInt(), y.getAsInt(), 20, "menu.multiplayer");

      x = (JsonPrimitive) Consts.background.get(8);
      y = (JsonPrimitive) Consts.background.get(7);
      gui.CreatebuttonwithoutImage(minecraft, titlescreen, new OptionsScreen(titlescreen, minecraft.options),
            buttonwidth, j, x.getAsInt(), y.getAsInt(), 20, "menu.options");

      x = (JsonPrimitive) Consts.background.get(10);
      y = (JsonPrimitive) Consts.background.get(9);
      gui.CreatebuttonwithoutImageQuit(minecraft, titlescreen, buttonwidth, j, x.getAsInt(), y.getAsInt(), 20,
            "menu.quit");
   ...
   private int CalculateY(int input, int j) {
      int y = 0;
      // allows me to detect when a number in my config is - and turns it from neg to
      // postive and subtracts it
      if (input < 0) {
         y = j - Math.abs(input);
      } else {
         y = j + input;
