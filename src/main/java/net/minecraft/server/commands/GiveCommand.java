...
public class GiveCommand
{
    ...
    private static int giveItem(CommandSourceStack pSource, ItemInput pItem, Collection<ServerPlayer> pTargets, int pCount) throws CommandSyntaxException
    {
        ...
        if (pCount > j)
        {
            pSource.sendFailure(Component.a("commands.give.failed.toomanyitems", j, pItem.createItemStack(pCount, false).getDisplayName()));
            return 0;
        }
        else
        {
            ...
            if (pTargets.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.give.success.single", pCount, pItem.createItemStack(pCount, false).getDisplayName(), pTargets.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.give.success.single", pCount, pItem.createItemStack(pCount, false).getDisplayName(), pTargets.size()), true);
            }
