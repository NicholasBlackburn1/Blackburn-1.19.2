...
public class RecipeCommand
{
    ...
    private static int giveRecipes(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, Collection < Recipe<? >> pRecipes) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pTargets.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.recipe.give.success.single", pRecipes.size(), pTargets.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.recipe.give.success.multiple", pRecipes.size(), pTargets.size()), true);
            }
    ...
    private static int takeRecipes(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, Collection < Recipe<? >> pRecipes) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pTargets.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.recipe.take.success.single", pRecipes.size(), pTargets.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.recipe.take.success.multiple", pRecipes.size(), pTargets.size()), true);
            }
