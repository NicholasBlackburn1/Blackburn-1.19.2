...
public class StorageDataAccessor implements DataAccessor
{
    ...
    {
        return Component.a("commands.data.storage.modified", this.id);
    }

    public Component getPrintSuccess(Tag pNbt)
    {
        return Component.a("commands.data.storage.query", this.id, NbtUtils.toPrettyComponent(pNbt));
    }

    public Component getPrintSuccess(NbtPathArgument.NbtPath pPath, double pScale, int p_139552_)
    {
        return Component.a("commands.data.storage.get", pPath, this.id, String.format(Locale.ROOT, "%.2f", pScale), p_139552_);
    }
}
