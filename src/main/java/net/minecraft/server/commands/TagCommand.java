...
public class TagCommand
{
    ...
    private static int addTag(CommandSourceStack pSource, Collection <? extends Entity > pEntities, String pTagName) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pEntities.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.tag.add.success.single", pTagName, pEntities.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.tag.add.success.multiple", pTagName, pEntities.size()), true);
            }
    ...
    private static int removeTag(CommandSourceStack pSource, Collection <? extends Entity > pEntities, String pTagName) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pEntities.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.tag.remove.success.single", pTagName, pEntities.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.tag.remove.success.multiple", pTagName, pEntities.size()), true);
            }
    ...
    private static int listTags(CommandSourceStack pSource, Collection <? extends Entity > pEntities)
    {
        ...
        if (pEntities.size() == 1)
        {
            ...
            if (set.isEmpty())
            {
                pSource.sendSuccess(Component.a("commands.tag.list.single.empty", entity1.getDisplayName()), false);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.tag.list.single.success", entity1.getDisplayName(), set.size(), ComponentUtils.formatList(set)), false);
            }
        }
        else if (set.isEmpty())
        {
            pSource.sendSuccess(Component.a("commands.tag.list.multiple.empty", pEntities.size()), false);
        }
        else
        {
            pSource.sendSuccess(Component.a("commands.tag.list.multiple.success", pEntities.size(), set.size(), ComponentUtils.formatList(set)), false);
        }
