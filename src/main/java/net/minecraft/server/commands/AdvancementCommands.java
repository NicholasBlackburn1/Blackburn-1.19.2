...
public class AdvancementCommands
{
    ...
    private static int perform(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, AdvancementCommands.Action pAction, Collection<Advancement> pAdvancements)
    {
        ...
        if (i == 0)
        {
            if (pAdvancements.size() == 1)
            {
                if (pTargets.size() == 1)
                {
                    throw new CommandRuntimeException(Component.a(pAction.getKey() + ".one.to.one.failure", pAdvancements.iterator().next().getChatComponent(), pTargets.iterator().next().getDisplayName()));
                }
                else
                {
                    throw new CommandRuntimeException(Component.a(pAction.getKey() + ".one.to.many.failure", pAdvancements.iterator().next().getChatComponent(), pTargets.size()));
                }
            }
            else if (pTargets.size() == 1)
            {
                throw new CommandRuntimeException(Component.a(pAction.getKey() + ".many.to.one.failure", pAdvancements.size(), pTargets.iterator().next().getDisplayName()));
            }
            else
            {
                throw new CommandRuntimeException(Component.a(pAction.getKey() + ".many.to.many.failure", pAdvancements.size(), pTargets.size()));
            }
        }
        else
        {
            if (pAdvancements.size() == 1)
            {
                if (pTargets.size() == 1)
                {
                    pSource.sendSuccess(Component.a(pAction.getKey() + ".one.to.one.success", pAdvancements.iterator().next().getChatComponent(), pTargets.iterator().next().getDisplayName()), true);
                }
                else
                {
                    pSource.sendSuccess(Component.a(pAction.getKey() + ".one.to.many.success", pAdvancements.iterator().next().getChatComponent(), pTargets.size()), true);
                }
            }
            else if (pTargets.size() == 1)
            {
                pSource.sendSuccess(Component.a(pAction.getKey() + ".many.to.one.success", pAdvancements.size(), pTargets.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a(pAction.getKey() + ".many.to.many.success", pAdvancements.size(), pTargets.size()), true);
            }
    ...
    private static int performCriterion(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, AdvancementCommands.Action pAction, Advancement pAdvancement, String pCriterionName)
    {
        ...
        {
            throw new CommandRuntimeException(Component.a("commands.advancement.criterionNotFound", pAdvancement.getChatComponent(), pCriterionName));
        }
        else
        {
            for (ServerPlayer serverplayer : pTargets)
            ...
            if (i == 0)
            {
                if (pTargets.size() == 1)
                {
                    throw new CommandRuntimeException(Component.a(pAction.getKey() + ".criterion.to.one.failure", pCriterionName, pAdvancement.getChatComponent(), pTargets.iterator().next().getDisplayName()));
                }
                else
                {
                    throw new CommandRuntimeException(Component.a(pAction.getKey() + ".criterion.to.many.failure", pCriterionName, pAdvancement.getChatComponent(), pTargets.size()));
                }
            }
            else
            {
                if (pTargets.size() == 1)
                {
                    pSource.sendSuccess(Component.a(pAction.getKey() + ".criterion.to.one.success", pCriterionName, pAdvancement.getChatComponent(), pTargets.iterator().next().getDisplayName()), true);
                }
                else
                {
                    pSource.sendSuccess(Component.a(pAction.getKey() + ".criterion.to.many.success", pCriterionName, pAdvancement.getChatComponent(), pTargets.size()), true);
                }
