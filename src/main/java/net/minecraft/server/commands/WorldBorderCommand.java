...
public class WorldBorderCommand
{
    ...
    private static final SimpleCommandExceptionType ERROR_TOO_BIG = new SimpleCommandExceptionType(Component.a("commands.worldborder.set.failed.big", 5.9999968E7D));
    private static final SimpleCommandExceptionType ERROR_TOO_FAR_OUT = new SimpleCommandExceptionType(Component.a("commands.worldborder.set.failed.far", 2.9999984E7D));
    ...
    private static int setDamageBuffer(CommandSourceStack pSource, float pDistance) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            pSource.sendSuccess(Component.a("commands.worldborder.damage.buffer.success", String.format(Locale.ROOT, "%.2f", pDistance)), true);
    ...
    private static int setDamageAmount(CommandSourceStack pSource, float pDamagePerBlock) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            pSource.sendSuccess(Component.a("commands.worldborder.damage.amount.success", String.format(Locale.ROOT, "%.2f", pDamagePerBlock)), true);
    ...
    private static int setWarningTime(CommandSourceStack pSource, int pTime) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            pSource.sendSuccess(Component.a("commands.worldborder.warning.time.success", pTime), true);
            return pTime;
        }
    }
    ...
    private static int setWarningDistance(CommandSourceStack pSource, int pDistance) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            pSource.sendSuccess(Component.a("commands.worldborder.warning.distance.success", pDistance), true);
            return pDistance;
        }
    }
    ...
    private static int getSize(CommandSourceStack pSource)
    {
        ...
        pSource.sendSuccess(Component.a("commands.worldborder.get", String.format(Locale.ROOT, "%.0f", d0)), false);
    ...
    private static int setCenter(CommandSourceStack pSource, Vec2 pPos) throws CommandSyntaxException
    {
        ...
        else if (!((double)Math.abs(pPos.x) > 2.9999984E7D) && !((double)Math.abs(pPos.y) > 2.9999984E7D))
        {
            ...
            pSource.sendSuccess(Component.a("commands.worldborder.center.success", String.format(Locale.ROOT, "%.2f", pPos.x), String.format(Locale.ROOT, "%.2f", pPos.y)), true);
            return 0;
        }
        else
        {
    ...
    private static int setSize(CommandSourceStack pSource, double pNewSize, long p_139255_) throws CommandSyntaxException
    {
        ...
        else
        {
            if (p_139255_ > 0L)
            {
                ...
                if (pNewSize > d0)
                {
                    pSource.sendSuccess(Component.a("commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", pNewSize), Long.toString(p_139255_ / 1000L)), true);
                }
                else
                {
                    pSource.sendSuccess(Component.a("commands.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", pNewSize), Long.toString(p_139255_ / 1000L)), true);
                }
            }
            else
            {
                worldborder.setSize(pNewSize);
                pSource.sendSuccess(Component.a("commands.worldborder.set.immediate", String.format(Locale.ROOT, "%.1f", pNewSize)), true);
            }
