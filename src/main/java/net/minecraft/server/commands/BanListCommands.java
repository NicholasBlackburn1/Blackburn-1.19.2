...
public class BanListCommands
{
    ...
    private static int showList(CommandSourceStack pSource, Collection <? extends BanListEntry<? >> pBannedPlayerList)
    {
        ...
        {
            pSource.sendSuccess(Component.a("commands.banlist.list", pBannedPlayerList.size()), false);

            for (BanListEntry<?> banlistentry : pBannedPlayerList)
            {
                pSource.sendSuccess(Component.a("commands.banlist.entry", banlistentry.getDisplayName(), banlistentry.getSource(), banlistentry.getReason()), false);
            }
        }
