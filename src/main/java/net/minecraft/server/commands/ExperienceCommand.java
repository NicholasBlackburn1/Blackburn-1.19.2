...
public class ExperienceCommand
{
    ...
    private static int queryExperience(CommandSourceStack pSource, ServerPlayer pPlayer, ExperienceCommand.Type pType)
    {
        ...
        pSource.sendSuccess(Component.a("commands.experience.query." + pType.name, pPlayer.getDisplayName(), i), false);
        return i;
    }
    ...
    private static int addExperience(CommandSourceStack pSource, Collection <? extends ServerPlayer > pTargets, int pAmount, ExperienceCommand.Type pType)
    {
        ...
        if (pTargets.size() == 1)
        {
            pSource.sendSuccess(Component.a("commands.experience.add." + pType.name + ".success.single", pAmount, pTargets.iterator().next().getDisplayName()), true);
        }
        else
        {
            pSource.sendSuccess(Component.a("commands.experience.add." + pType.name + ".success.multiple", pAmount, pTargets.size()), true);
        }
    ...
    private static int setExperience(CommandSourceStack pSource, Collection <? extends ServerPlayer > pTargets, int pAmount, ExperienceCommand.Type pType) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pTargets.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.experience.set." + pType.name + ".success.single", pAmount, pTargets.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.experience.set." + pType.name + ".success.multiple", pAmount, pTargets.size()), true);
            }
