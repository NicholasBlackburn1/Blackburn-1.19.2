...
public class BanIpCommands
{
    ...
    private static int banIp(CommandSourceStack pSource, String pIp, @Nullable Component pReason) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            pSource.sendSuccess(Component.a("commands.banip.success", pIp, ipbanlistentry.getReason()), true);

            if (!list.isEmpty())
            {
                pSource.sendSuccess(Component.a("commands.banip.info", list.size(), EntitySelector.joinNames(list)), true);
            }
