...
public class KillCommand
{
    ...
    private static int kill(CommandSourceStack pSource, Collection <? extends Entity > pTargets)
    {
        ...
        if (pTargets.size() == 1)
        {
            pSource.sendSuccess(Component.a("commands.kill.success.single", pTargets.iterator().next().getDisplayName()), true);
        }
        else
        {
            pSource.sendSuccess(Component.a("commands.kill.success.multiple", pTargets.size()), true);
        }
