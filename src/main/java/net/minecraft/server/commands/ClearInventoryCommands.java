    ...
    private static int clearInventory(CommandSourceStack pSource, Collection<ServerPlayer> pTargetPlayers, Predicate<ItemStack> pItemPredicate, int pMaxCount) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pMaxCount == 0)
            {
                if (pTargetPlayers.size() == 1)
                {
                    pSource.sendSuccess(Component.a("commands.clear.test.single", i, pTargetPlayers.iterator().next().getDisplayName()), true);
                }
                else
                {
                    pSource.sendSuccess(Component.a("commands.clear.test.multiple", i, pTargetPlayers.size()), true);
                }
            }
            else if (pTargetPlayers.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.clear.success.single", i, pTargetPlayers.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.clear.success.multiple", i, pTargetPlayers.size()), true);
            }
