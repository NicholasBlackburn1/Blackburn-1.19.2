...
public class EnchantCommand
{
    ...
    {
        return Component.a("commands.enchant.failed.level", p_137022_, p_137023_);
    });
    ...
    private static int enchant(CommandSourceStack pSource, Collection <? extends Entity > pTargets, Enchantment pEnchantment, int pLevel) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            else
            {
                if (pTargets.size() == 1)
                {
                    pSource.sendSuccess(Component.a("commands.enchant.success.single", pEnchantment.getFullname(pLevel), pTargets.iterator().next().getDisplayName()), true);
                }
                else
                {
                    pSource.sendSuccess(Component.a("commands.enchant.success.multiple", pEnchantment.getFullname(pLevel), pTargets.size()), true);
                }
