...
public class EffectCommands
{
    ...
    private static int giveEffect(CommandSourceStack pSource, Collection <? extends Entity > pTargets, MobEffect pEffect, @Nullable Integer pSeconds, int pAmplifier, boolean pShowParticles) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pTargets.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.effect.give.success.single", pEffect.getDisplayName(), pTargets.iterator().next().getDisplayName(), j / 20), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.effect.give.success.multiple", pEffect.getDisplayName(), pTargets.size(), j / 20), true);
            }
    ...
    private static int clearEffects(CommandSourceStack pSource, Collection <? extends Entity > pTargets) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pTargets.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.effect.clear.everything.success.single", pTargets.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.effect.clear.everything.success.multiple", pTargets.size()), true);
            }
    ...
    private static int clearEffect(CommandSourceStack pSource, Collection <? extends Entity > pTargets, MobEffect pEffect) throws CommandSyntaxException
    {
        ...
        else
        {
            if (pTargets.size() == 1)
            {
                pSource.sendSuccess(Component.a("commands.effect.clear.specific.success.single", pEffect.getDisplayName(), pTargets.iterator().next().getDisplayName()), true);
            }
            else
            {
                pSource.sendSuccess(Component.a("commands.effect.clear.specific.success.multiple", pEffect.getDisplayName(), pTargets.size()), true);
            }
