...
public class TitleCommand
{
    ...
    private static int clearTitle(CommandSourceStack pSource, Collection<ServerPlayer> pTargets)
    {
        ...
        if (pTargets.size() == 1)
        {
            pSource.sendSuccess(Component.a("commands.title.cleared.single", pTargets.iterator().next().getDisplayName()), true);
        }
        else
        {
            pSource.sendSuccess(Component.a("commands.title.cleared.multiple", pTargets.size()), true);
        }
    ...
    private static int resetTitle(CommandSourceStack pSource, Collection<ServerPlayer> pTargets)
    {
        ...
        if (pTargets.size() == 1)
        {
            pSource.sendSuccess(Component.a("commands.title.reset.single", pTargets.iterator().next().getDisplayName()), true);
        }
        else
        {
            pSource.sendSuccess(Component.a("commands.title.reset.multiple", pTargets.size()), true);
        }
    ...
    private static int showTitle(CommandSourceStack p_142781_, Collection<ServerPlayer> p_142782_, Component p_142783_, String p_142784_, Function < Component, Packet<? >> p_142785_) throws CommandSyntaxException
    {
        ...
        if (p_142782_.size() == 1)
        {
            p_142781_.sendSuccess(Component.a("commands.title.show." + p_142784_ + ".single", p_142782_.iterator().next().getDisplayName()), true);
        }
        else
        {
            p_142781_.sendSuccess(Component.a("commands.title.show." + p_142784_ + ".multiple", p_142782_.size()), true);
        }
    ...
    private static int setTimes(CommandSourceStack pSource, Collection<ServerPlayer> pTarget, int pFade, int pStay, int pFadeOut)
    {
        ...
        if (pTarget.size() == 1)
        {
            pSource.sendSuccess(Component.a("commands.title.times.single", pTarget.iterator().next().getDisplayName()), true);
        }
        else
        {
            pSource.sendSuccess(Component.a("commands.title.times.multiple", pTarget.size()), true);
        }
