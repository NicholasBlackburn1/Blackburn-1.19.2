...
public class GameModeCommand
{
    ...
    private static void logGamemodeChange(CommandSourceStack pSource, ServerPlayer pPlayer, GameType pGameType)
    {
        ...
        {
            pSource.sendSuccess(Component.a("commands.gamemode.success.self", component), true);
        }
        else
        {
            if (pSource.getLevel().getGameRules().getBoolean(GameRules.RULE_SENDCOMMANDFEEDBACK))
            {
                pPlayer.sendSystemMessage(Component.a("gameMode.changed", component));
            }

            pSource.sendSuccess(Component.a("commands.gamemode.success.other", pPlayer.getDisplayName(), component), true);
        }
    }
