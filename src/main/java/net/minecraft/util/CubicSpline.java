...
public interface CubicSpline<C, I extends ToFloatFunction<C>> extends ToFloatFunction<C>
{
    ...
    static <C, I extends ToFloatFunction<C>> Codec<CubicSpline<C, I>> codec(Codec<I> p_184263_)
    {
        ...
        Codec<CubicSpline.Multipoint<C, I>> codec1 = RecordCodecBuilder.create((p_184267_) ->
        {
            return p_184267_.group(p_184263_.fieldOf("coordinate").forGetter(CubicSpline.Multipoint::coordinate), ExtraCodecs.nonEmptyList(codec.listOf()).fieldOf("points").forGetter((p_184272_) -> {
                ...
                {
                    Point<C, I> point = (Point)p_184259_.get(i);
                    afloat[i] = point.location();
                    builder.add(point.value());
                    afloat1[i] = point.derivative();
                }

                return CubicSpline.Multipoint.a(p_184258_, afloat, builder.build(), afloat1);
            });
        });
    ...
    public static final class Builder<C, I extends ToFloatFunction<C>>
    {
        ...
        public CubicSpline<C, I> build()
        {
            ...
            {
                return CubicSpline.Multipoint.a(this.coordinate, this.locations.toFloatArray(), ImmutableList.copyOf(this.values), this.derivatives.toFloatArray());
            }
        }
    }
    ...
    {
        public Multipoint {
            a(locations, values, derivatives);
        }

        static <C, I extends ToFloatFunction<C>> CubicSpline.Multipoint<C, I> a(I p_216144_, float[] p_216145_, List<CubicSpline<C, I>> p_216146_, float[] p_216147_)
        {
            a(p_216145_, p_216146_, p_216147_);
            ...
            if (f2 < p_216145_[0])
            {
                float f4 = a(f2, p_216145_, p_216146_.get(0).minValue(), p_216147_, 0);
                float f5 = a(f2, p_216145_, p_216146_.get(0).maxValue(), p_216147_, 0);
            ...
            if (f3 > p_216145_[i])
            {
                float f24 = a(f3, p_216145_, p_216146_.get(i).minValue(), p_216147_, i);
                float f25 = a(f3, p_216145_, p_216146_.get(i).maxValue(), p_216147_, i);
        ...
        private static float a(float p_216134_, float[] p_216135_, float p_216136_, float[] p_216137_, int p_216138_)
        {
            float f = p_216137_[p_216138_];
            return f == 0.0F ? p_216136_ : p_216136_ + f * (p_216134_ - p_216135_[p_216138_]);
        }

        private static <C, I extends ToFloatFunction<C>> void a(float[] p_216152_, List<CubicSpline<C, I>> p_216153_, float[] p_216154_)
        {
        ...
        public float apply(C p_184340_)
        {
            ...
            int i = a(this.locations, f);
            int j = this.locations.length - 1;

            if (i < 0)
            {
                return a(f, this.locations, this.values.get(0).apply(p_184340_), this.derivatives, 0);
            }
            else if (i == j)
            {
                return a(f, this.locations, this.values.get(j).apply(p_184340_), this.derivatives, j);
            }
            else
            {
        ...
        private static int a(float[] p_216149_, float p_216150_)
        {
        ...
        {
            return a(p_211585_.visit(this.coordinate), this.locations, this.values().stream().map((p_211588_) ->
            {
