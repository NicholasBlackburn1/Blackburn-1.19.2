...
public record GcHeapStat(Instant timestamp, long heapUsed, GcHeapStat.Timing timing)
{
    ...
    private static double calculateAllocationRatePerSecond(List<GcHeapStat> p_185696_)
    {
        ...
        {
            GcHeapStat gcheapstat = (GcHeapStat)list.get(j);
            GcHeapStat gcheapstat1 = (GcHeapStat)list1.get(j - 1);
            i += gcheapstat.heapUsed - gcheapstat1.heapUsed;
        }

        Duration duration = Duration.between(((GcHeapStat)p_185696_.get(1)).timestamp, ((GcHeapStat)p_185696_.get(p_185696_.size() - 1)).timestamp);
