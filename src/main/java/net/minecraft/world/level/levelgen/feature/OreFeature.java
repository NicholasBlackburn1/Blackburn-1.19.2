...
public class OreFeature extends Feature<OreConfiguration>
{
    ...
    protected boolean doPlace(WorldGenLevel pLevel, RandomSource pRandom, OreConfiguration pConfig, double pMinX, double p_225176_, double pMaxX, double p_225178_, double pMinZ, double p_225180_, int pMaxZ, int p_225182_, int pMinY, int p_225184_, int pMaxY)
    {
        int i = 0;
        BitSet bitset = new BitSet(p_225184_ * pMaxY * p_225184_);
        ...
        for (int k = 0; k < j; ++k)
        {
            ...
            double d0 = Mth.lerp((double)f, pMinX, p_225176_);
            double d1 = Mth.lerp((double)f, pMinZ, p_225180_);
            double d2 = Mth.lerp((double)f, pMaxX, p_225178_);
                            ...
                            for (int j2 = l; j2 <= k1; ++j2)
                            {
                                ...
                                if (d5 * d5 + d6 * d6 < 1.0D)
                                {
                                    for (int k2 = i1; k2 <= l1; ++k2)
                                    {
                                        ...
                                        {
                                            int l2 = i2 - pMaxZ + (j2 - p_225182_) * p_225184_ + (k2 - pMinY) * p_225184_ * pMaxY;
    ...
    public static boolean canPlaceOre(BlockState pState, Function<BlockPos, BlockState> pAdjacentStateAccessor, RandomSource pRandom, OreConfiguration pConfig, OreConfiguration.TargetBlockState pTargetState, BlockPos.MutableBlockPos pMatablePos)
    {
