...
public class DripstoneClusterFeature extends Feature<DripstoneClusterConfiguration>
{
    ...
    private void placeColumn(WorldGenLevel pLevel, RandomSource pRandom, BlockPos pPos, int pX, int pZ, float pWetness, double pChance, int p_225023_, float pHeight, DripstoneClusterConfiguration pDensity)
    {
        ...
        if (optional.isPresent())
        {
            ...
            if (optionalint.isPresent() || optionalint1.isPresent())
            {
                ...
                if (optionalint2.isPresent() && flag2 && !this.isLava(pLevel, pPos.atY(optionalint2.getAsInt())))
                {
                    ...
                    {
                        i3 = this.getDripstoneHeight(pRandom, pX, pZ, pHeight, p_225023_, pDensity);
                    }
                }
                else
                {
