...
public class IcebergFeature extends Feature<BlockStateConfiguration>
{
    ...
    private void generateCutOut(RandomSource pRandom, LevelAccessor pLevel, int pMajorAxis, int pHeight, BlockPos pPos, boolean pElliptical, int pEllipseRadius, double pAngle, int p_225108_)
    {
        ...
        for (int i1 = 0; i1 < pHeight - 3; ++i1)
        {
            ...
            this.carve(j1, i1, pPos, pLevel, false, d0, blockpos, pEllipseRadius, p_225108_);
        }

        for (int k1 = -1; k1 > -pHeight + pRandom.nextInt(5); --k1)
        {
            int l1 = this.heightDependentRadiusSteep(pRandom, -k1, pHeight, pMajorAxis);
            this.carve(l1, k1, pPos, pLevel, true, d0, blockpos, pEllipseRadius, p_225108_);
        }
    }
    ...
    private void generateIcebergBlock(LevelAccessor pLevel, RandomSource pRandom, BlockPos pPos, int pHeight, int pLocalX, int pLocalY, int pLocalZ, int pRadius, int pMajorRadius, boolean pElliptical, int pMinorRadius, double pAngle, boolean p_225122_, BlockState pPlaceSnow)
    {
        ...
        if (d0 < 0.0D)
        {
            ...
            this.setIcebergBlock(blockpos, pLevel, pRandom, pHeight - pLocalY, pHeight, pElliptical, p_225122_, pPlaceSnow);
        }
    }
