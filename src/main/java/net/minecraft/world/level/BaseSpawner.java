...
public abstract class BaseSpawner
{
                ...
                for (int i = 0; i < this.spawnCount; ++i)
                {
                    ...
                    if (pServerLevel.noCollision(optional.get().getAABB(d0, d1, d2)))
                    {
                        ...
                        if (this.nextSpawnData.getCustomSpawnRules().isPresent())
                        {
                            ...
                            SpawnData.CustomSpawnRules spawndata$customspawnrules = (SpawnData.CustomSpawnRules)this.nextSpawnData.getCustomSpawnRules().get();
    ...
    public void load(@Nullable Level pLevel, BlockPos pPos, CompoundTag pTag)
    {
        ...
        else
        {
            ...
            if (flag1)
            {
                SpawnData spawndata1 = (SpawnData)SpawnData.CODEC.parse(NbtOps.INSTANCE, pTag.getCompound("SpawnData")).resultOrPartial((p_186380_) ->
                {
            ...
            else
            {
                ...
                {
                    this.setNextSpawnData(pLevel, pPos, (SpawnData)p_186378_.getData());
                });
            }
        }
