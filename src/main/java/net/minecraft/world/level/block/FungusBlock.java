...
public class FungusBlock extends BushBlock implements BonemealableBlock
{
    ...
    {
        Block block = ((HugeFungusConfiguration)((ConfiguredFeature)this.feature.get().value()).config()).validBaseState.getBlock();
    ...
    {
        ((ConfiguredFeature)this.feature.get().value()).place(pLevel, pLevel.getChunkSource().getGenerator(), pRandom, pPos);
    }
}
