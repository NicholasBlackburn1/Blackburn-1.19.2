...
public class LevelStorageSource
{
    ...
    @Nullable
    <T> T readLevelData(LevelStorageSource.LevelDirectory pSaveDir, BiFunction<Path, DataFixer, T> pLevelDatReader)
    {
        if (!Files.exists(pSaveDir.path()))
        {
            return (T)null;
        }
        else
        {
            Path path = pSaveDir.dataFile();
            ...
            path = pSaveDir.oldDataFile();
            return (T)(Files.exists(path) ? pLevelDatReader.apply(path, this.fixerUpper) : null);
        }
    }

    @Nullable
    private static DataPackConfig getDataPacks(Path pLevelDat, DataFixer pFixer)
    {
        try
        {
    ...
    BiFunction<Path, DataFixer, LevelSummary> levelSummaryReader(LevelStorageSource.LevelDirectory pSaveDir, boolean pLocked)
    {
            ...
            try {
                ...
                if (tag instanceof CompoundTag compoundtag)
                {
                    ...
                    if (j == 19132 || j == 19133)
                    {
                        ...
                        return new LevelSummary(levelsettings, levelversion, pSaveDir.directoryName(), flag, pLocked, path);
                    }
                }
                else {
