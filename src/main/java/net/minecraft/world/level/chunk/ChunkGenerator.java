...
public abstract class ChunkGenerator
{
    ...
    public ChunkGenerator(Registry<StructureSet> pBiomeSource, Optional<HolderSet<StructureSet>> pRuntimeBiomeSource, BiomeSource pSettings, Function<Holder<Biome>, BiomeGenerationSettings> pStrongholdSeed)
    {
        this.structureSets = pBiomeSource;
        this.biomeSource = pSettings;
        this.generationSettingsGetter = pStrongholdSeed;
        this.structureOverrides = pRuntimeBiomeSource;
        this.featuresPerStep = Suppliers.memoize(() ->
        {
            return FeatureSorter.buildFeaturesPerStep(List.copyOf(pSettings.possibleBiomes()), (p_223216_) -> {
                return pStrongholdSeed.apply(p_223216_).features();
            }, true);
        });
    }
    ...
    private void generatePositions(RandomState p_223228_)
    {
        ...
        {
            StructureSet structureset = (StructureSet)p_223208_.value();
            boolean flag = false;
                    ...
                    if (k < j)
                    {
                        ...
                        for (Holder<Biome> holder : set)
                        {
                            ...
                            if (k < list1.size())
                            {
                                ...
                                FeatureSorter.StepFeatureData featuresorter$stepfeaturedata1 = (FeatureSorter.StepFeatureData)list.get(k);
                        ...
                        Arrays.sort(aint);
                        FeatureSorter.StepFeatureData featuresorter$stepfeaturedata = (FeatureSorter.StepFeatureData)list.get(k);
    ...
    {
        StructureSet structureset = (StructureSet)p_223142_.value();
    ...
    public WeightedRandomList<MobSpawnSettings.SpawnerData> getMobsAt(Holder<Biome> pBiome, StructureManager pStructureFeatureManager, MobCategory pCategory, BlockPos pPos)
    {
        Map<Structure, LongSet> map = pStructureFeatureManager.getAllStructuresAt(pPos);

        for (Map.Entry<Structure, LongSet> entry : map.entrySet())
        {
            Structure structure = entry.getKey();
            StructureSpawnOverride structurespawnoverride = (StructureSpawnOverride)structure.spawnOverrides().get(pCategory);
            ...
            if (structurespawnoverride != null)
            {
                ...
                {
                    return pStructureFeatureManager.structureHasPieceAt(pPos, p_223065_);
                } : (p_223130_) ->
                {
                    return p_223130_.getBoundingBox().isInside(pPos);
                };
                pStructureFeatureManager.fillStartsForStructure(structure, entry.getValue(), (p_223220_) ->
                {
    ...
    public void createStructures(RegistryAccess p_223165_, RandomState p_223166_, StructureManager p_223167_, ChunkAccess p_223168_, StructureTemplateManager p_223169_, long p_223170_)
    {
        ...
        {
            StructurePlacement structureplacement = ((StructureSet)p_223075_.value()).placement();
            List<StructureSet.StructureSelectionEntry> list = ((StructureSet)p_223075_.value()).structures();
            ...
            if (structureplacement.isStructureChunk(this, p_223166_, p_223170_, chunkpos.x, chunkpos.z))
            {
                if (list.size() == 1)
                {
                    this.tryGenerateStructure((StructureSet.StructureSelectionEntry)list.get(0), p_223167_, p_223165_, p_223166_, p_223169_, p_223170_, p_223168_, chunkpos, sectionpos);
                }
                else
                {
    ...
    public void createReferences(WorldGenLevel pLevel, StructureManager pStructureFeatureManager, ChunkAccess pChunk)
    {
        int i = 8;
            ...
            for (int k1 = k - 8; k1 <= k + 8; ++k1)
            {
                ...
                for (StructureStart structurestart : pLevel.getChunk(j1, k1).getAllStarts().values())
                {
                    try
                    {
                        ...
                        {
                            pStructureFeatureManager.addReferenceForStructure(sectionpos, structurestart.getStructure(), l1, pChunk);
