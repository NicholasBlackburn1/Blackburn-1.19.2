        ...
        if (this.type == EntityType.TROPICAL_FISH)
        {
            ...
            if (compoundtag != null && compoundtag.contains("BucketVariantTag", 3))
            {
                ...
                for (int j = 0; j < TropicalFish.COMMON_VARIANTS.length; ++j)
                {
                    ...
                    {
                        pTooltipComponents.add(Component.translatable(TropicalFish.getPredefinedName(j)).a(achatformatting));
                        return;
                    }
                }

                pTooltipComponents.add(Component.translatable(TropicalFish.getFishTypeName(i)).a(achatformatting));
