...
public class RecipeButton extends AbstractWidget
{
    ...
    {
        ItemStack itemstack = this.getOrderedRecipes().get(this.currentIndex).getResultItem();
        pNarrationElementOutput.add(NarratedElementType.TITLE, Component.a("narration.recipe", itemstack.getHoverName()));

        if (this.collection.getRecipes(this.book.isFiltering(this.menu)).size() > 1)
        {
            pNarrationElementOutput.a(NarratedElementType.USAGE, Component.translatable("narration.button.usage.hovered"), Component.translatable("narration.recipe.usage.more"));
        }
        else
        {
