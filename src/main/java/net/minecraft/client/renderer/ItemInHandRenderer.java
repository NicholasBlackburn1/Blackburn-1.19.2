        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(pHand))
        {
            if (!pPlayer.isScoping())
            {
                ...
                else
                {
                    ...
                    if (!IClientItemExtensions.of(pStack).applyForgeHandTransform(pMatrixStack, this.minecraft.player, humanoidarm, pStack, pPartialTicks, pEquippedProgress, pSwingProgress))
                    {
                        if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand)
                        {
                            int k = flag1 ? 1 : -1;
