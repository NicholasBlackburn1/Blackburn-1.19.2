...
public class FaceBakery
{
    ...
    public BakedQuad bakeQuad(Vector3f pPosFrom, Vector3f pPosTo, BlockElementFace pFace, TextureAtlasSprite pSprite, Direction pFacing, ModelState pTransform, @Nullable BlockElementRotation pPartRotation, boolean pShade, ResourceLocation pModelLocation)
    {
        ...
        if (Reflector.ForgeHooksClient_fillNormal.exists())
        {
            ...
            BakedQuad bakedquad = new BakedQuad(aint, pFace.tintIndex, direction, pSprite, pShade);

            if (Reflector.QuadTransformers_settingEmissivity.exists() && Reflector.ForgeBlockElementFace_emissivity.exists())
            {
                int i = Reflector.getFieldValueInt(pFace, Reflector.ForgeBlockElementFace_emissivity, -1);

                if (i >= 0)
                {
                    Object object = Reflector.QuadTransformers_settingEmissivity.call((int)i);
