            ...
            try
            {
                ...
                if (this.options.entityShadows().get() && this.shouldRenderShadow && entityrenderer.shadowRadius > 0.0F && !pEntity.isInvisible())
                {
                    ...
                    if (f > 0.0F)
                    {
                        ...
                        if (flag)
                        {
                            p_114391_.translate((double)entityrenderer.shadowOffsetX, 0.0D, (double)entityrenderer.shadowOffsetZ);
                        }
    ...
    private static void renderBlockShadow(PoseStack.Pose pMatrixEntry, VertexConsumer pBuffer, LevelReader pLevel, BlockPos pBlockPos, double pX, double p_114437_, double pY, float p_114439_, float pZ)
    {
        ...
        if (blockstate.getRenderShape() != RenderShape.INVISIBLE && pLevel.getMaxLocalRawBrightness(pBlockPos) > 3 && blockstate.isCollisionShapeFullBlock(pLevel, blockpos))
        {
            ...
            if (!voxelshape.isEmpty())
            {
                ...
                float f1 = (float)(((double)pZ - (p_114437_ - (double)pBlockPos.getY()) / 2.0D) * 0.5D * (double)f);
                ...
                if (f1 >= 0.0F)
                {
                    ...
                    float f5 = (float)(d3 - pY);
                    float f6 = (float)(d4 - pY);
